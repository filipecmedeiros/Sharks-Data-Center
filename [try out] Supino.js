...
//Escalas
var dataYinterval = d3.extent(dataset, d=>d["Supino"]);
var yScale = d3.scaleLinear().domain(dataYinterval).range([width-margin.left,0]);

var dataXinterval = ["QB", "OL", "DL", "RB", "LB", "TE", "DB", "WR", "x"];
var xScale = d3.scaleOrdinal().domain(dataXinterval).range([0, 50, 100, 150, 200, 250, 300, 350, 400, height-margin.top]);
...
.append('circle')
.attr("cy",d=>yScale(d["Supino"]))
.attr("cx",d=>xScale(d["Posição"]))
.attr("r",5);
...
.append('text')
.text(d=>d["Candidato"])
.attr('x', d=>xScale(d["Posição"]))
.attr('y', d=>yScale(d["Supino"])-5)
.attr('display', 'none');
